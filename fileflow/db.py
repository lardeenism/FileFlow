"""Database abstraction for Supabase PostgreSQL and MySQL."""
...
def backend():
    return current_app.config.get("DB_BACKEND", "supabase").strip().lower()
...
def _mysql_options(include_database=True):
    if pymysql is None:
        raise DatabaseError("PyMySQL is not installed. Run: python -m pip install -r requirements.txt")
    options = {
    ...
    if include_database:
        options["database"] = current_app.config["MYSQL_DATABASE"]
    return options
...
def _ensure_mysql_database():
    ...
    connection = None
    try:
        connection = pymysql.connect(**_mysql_options(include_database=False))
        ...
        raise DatabaseError(
            "Cannot connect to XAMPP MySQL. Start MySQL in the XAMPP Control Panel "
            "and verify the FILEFLOW_MYSQL_* settings."
        ) from error
    finally:
    ...
    if "db" not in g:
        try:
            if backend() in {"supabase", "postgres", "postgresql"}:
                ...
                g.db = pymysql.connect(**_mysql_options())
            else:
                raise DatabaseError(
                    "DB_BACKEND must be 'supabase', 'postgresql', or 'mysql'."
                )
...
def init_db():
    try:
        if backend() in {"supabase", "postgres", "postgresql"}:
            ...
            raise DatabaseError("DB_BACKEND must be 'supabase', 'postgresql', or 'mysql'.")
...
def fetch_all(query, params=()):
    try:
        ...
        if backend() == "mysql":
            with connection.cursor() as cursor:
                cursor.execute(_mysql_query(query), params)
                return list(cursor.fetchall())
        raise DatabaseError("DB_BACKEND must be 'supabase', 'postgresql', or 'mysql'.")
    except DatabaseError:
        raise
